...
VL53L0X_Error VL53L0X_WriteMulti(VL53L0X_DEV Dev, uint8_t index, uint8_t *pdata,
  ...
  // Data
  // Note: Needed to use i2c_master_write_byte as i2c_master_write will not
  // expect an ack after each byte
...
VL53L0X_Error VL53L0X_ReadMulti(VL53L0X_DEV Dev, uint8_t index, uint8_t *pdata,
  ...
  ESP_ERROR_CHECK(i2c_master_write_byte(
      cmd, (Dev->i2c_address << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN));
  // Write register
  ...
  // Write I2C address
  ESP_ERROR_CHECK(i2c_master_write_byte(
      cmd, (Dev->i2c_address << 1) | I2C_MASTER_READ, ACK_CHECK_EN));
